...
class CreateSpotForm(FlaskForm):
    name = StringField('name', validators=[DataRequired(message='Name is required'), Length(max=150, message='Spot name must be 150 characters or less')])
    address = StringField('address', validators=[Length(max=250, message='Address must be 250 characters or less')])
    city = StringField('city', validators=[DataRequired(message='City is required'), Length(max=100, message='Username must be 100 characters or less')])
    state = StringField('state', validators=[DataRequired(message='State is required'), Length(max=100, message='State must be 100 characters or less')])
